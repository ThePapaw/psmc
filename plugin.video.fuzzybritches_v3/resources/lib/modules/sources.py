...
class sources:
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.fuzzybritches_v3.container.items'

        self.metaProperty = 'plugin.video.fuzzybritches_v3.container.meta'
        ...
        self.module_name = 'Fuzzy Britches v3:'

        if externalEnabled == 'true':
            try:
                import fuzzybritchesscrapers
                self.sourceDict += fuzzybritchesscrapers.sources()
                self.module_name = 'fuzzybritchesscrapers (' + str(control.addon('script.module.fuzzybritchesv3scrapers').getSetting('package.folder')) + ' set):' \
                                   if control.addon('script.module.fuzzybritchesv3scrapers').getSetting('package.folder') != 'fuzzybritchesscrapers' else 'fuzzybritchesscrapers:'
            except:
                pass
