...
DEBUGPREFIX = '[ Fuzzy Britches v3 {0} | {1} | {2} | DEBUG ]'.format(version, kodi_version, sys_platform)
INFOPREFIX = '[ Fuzzy Britches v3 | INFO ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'Fuzzy Britches v3.log')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('Fuzzy Britches v3 Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
...
def upload_log():
    ...
    if not data:
        ...
        UserAgent = 'Fuzzy Britches v3 %s' % version
        try:
