...
if int(total_sizetext) > filesize:
	choice2 = xbmcgui.Dialog().yesno("[B][COLOR yellow]PSMC Maintenance[/COLOR][/B]", 'The packages folder is [COLOR lime]' + str(total_sizetext) +' MB [/COLOR] - [COLOR lime]' + str(count) + '[/COLOR] zip files', 'The folder can be cleaned to save space...', 'Do you want to clean it now?', yeslabel='Yes',nolabel='No')
	if choice2 == 1:
...
if int(total_sizetext2) > filesize_thumb:
	choice2 = xbmcgui.Dialog().yesno("[B][COLOR yellow]PSMC Maintenance[/COLOR][/B]", 'The thumbnail folder is [COLOR lime]' + str(total_sizetext2) + ' MB   [/COLOR]', 'The folder can be cleaned to save space...', 'Do you want to clean it now?', yeslabel='Yes',nolabel='No')
	if choice2 == 1:
