...
import re
import time
addon_id   = 'program.psmcmaintenance'
...
def TextBoxes(announce):
	class TextBox():
		...
		def setControls(self):
			self.win.getControl(self.CONTROL_LABEL).setLabel('PSMC - View Log Facility') # set heading
...
def TextBoxesPlain(announce):
	class TextBox():
		...
		def __init__(self,*args,**kwargs):
			...
			self.setControls()
		def setControls(self):
			self.win.getControl(self.CONTROL_LABEL).setLabel('[B]PSMC[/B]') # set heading
			try: f=open(announce); text=f.read()
			except: text=announce
			self.win.getControl(self.CONTROL_TEXTBOX).setText(str(text))
			return
...
def killkodi1():
	choice = xbmcgui.Dialog().yesno("[B][COLOR yellow]PSMC Maintenance[/COLOR][/B]", '     Do you want to Force Close your Media Center?', 'It is Highly recommended that you do!', '', yeslabel='[COLOR=red]Yes[/COLOR]',nolabel='[COLOR=green]No[/COLOR]')
	if choice == 0:
		return
	elif choice == 1:
		killkodi()
