...
def Backup():
    ...
    time.sleep(1)
    dialog.ok("[B][COLOR yellow]PSMC Maintenance[/COLOR][/B]", 'The backup was completed successfully!.',"Backup Location: ",'[COLOR=yellow]'+backup_zip+'[/COLOR]')
...
def skinswap():
	...
	if SKIN not in ['skin.confluence', 'skin.estuary']:
		...
		choice1 = xbmcgui.Dialog().yesno("[B][COLOR yellow]PSMC Maintenance[/COLOR][/B]", "The skin needs to be set back to [COLOR lime]%s[/COLOR]" % skin[5:], "Before doing a fresh install", "Would you like us to do that for you?", nolabel="No, Thanks", yeslabel="Yes, Swap Skin");
		if choice1 == 1:	
