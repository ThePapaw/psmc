...
EXCLUDES     = ['plugin.program.psmcmaintenance','plugin.program.psmcmaintenance','repository.fuzzybritches','backupdir','script.module.requests','temp','kodi.log','kodi.log.old','spmc.log','spmc.log.old','dbmc.log','dbmc.log.old']

thumbnailPath = xbmc.translatePath('special://userdata/Thumbnails');
cachePath = os.path.join(xbmc.translatePath('special://home'), 'cache')
HOME           =  xbmc.translatePath('special://home/')
tempPath = xbmc.translatePath('special://temp')
addonPath = os.path.join(os.path.join(xbmc.translatePath('special://home'), 'addons'),'plugin.program.psmcmaint')
...
def skincheck():
	if SKIN not in ['skin.confluence', 'skin.estuary']:
		...
		choice = xbmcgui.Dialog().yesno("[B][COLOR yellow]PSMC Maintenance[/COLOR][/B]", "The skin needs to be set back to [COLOR lime]%s[/COLOR]" % skin[5:], "Before doing a fresh install", "Would you like us to do that for you?", nolabel="No, Thanks", yeslabel="Yes, Swap Skin");
		if choice == 1:	
...
def freshstart():
    skincheck()
    choice = xbmcgui.Dialog().yesno("[COLOR=yellow]ABSOLUTELY CERTAIN????[/COLOR]", 'Are you absolutely certain you want to Start Fresh??', '', '[COLOR firebrick]All data, addons and settings[/COLOR] will be removed EXCLUDING this addon and Backups.', yeslabel='[COLOR=red]Yes[/COLOR]',nolabel='[COLOR=green]No[/COLOR]')
    if choice == 0:
        return
    elif choice == 1:
        dp.create("[B][COLOR yellow]PSMC Maintenance[/COLOR][/B]","Deleting data and preparing a new Fresh Start",'', 'Please Wait...')
        try:
...
def deleteAddonDB():
	...
	if codename == "Pass":
				...
				for name in files:
					if "addons" in name.lower():
						try:
							...
							dialog.ok(MaintTitle,'Error Removing ' + str(name),'','[COLOR yellow]Thank you for using PSMC Maintenance[/COLOR]')
							pass
					else:
						continue
		except:
		...
		dialog.ok(MaintTitle,'This feature is not available in Kodi 18','','[COLOR yellow]Thank you for using PSMC Maintenance[/COLOR]')
...
def deleteThumbnails(): 
    if os.path.exists(thumbnailPath)==True:  
            dialog = xbmcgui.Dialog()
            if dialog.yesno("[B][COLOR yellow]PSMC Maintenance[/COLOR][/B]", "This option deletes all Media Center thumbnails", "Are you sure you want to do this?"):
                    ...
                    if file_count > 0:                
                        for f in files:
                            try:
								...
								pass
		Remove_Textures()
		dialog.ok("[B][COLOR yellow]PSMC Maintenance[/COLOR][/B]", '                [COLOR=lime]All Thumbnails have been deleted.[/COLOR]','     [COLOR=red] Press OK to Attempt to Force Close your Media center[/COLOR]')
		Common.killkodi1()
...
def purgePackages():
    ...
    for root, dirs, files in os.walk(purgePath):
            file_count = 0
            file_count += len(files)
    if dialog.yesno("[B][COLOR yellow]PSMC Maintenance[/COLOR][/B]","This option deletes all Media Center Packages?", "%d packages found."%file_count, "Delete Them?"):  
        for root, dirs, files in os.walk(purgePath):
            file_count = 0
            file_count += len(files)
            if file_count > 0:            
...
def DeleteCrashLogs():  
	...
	if os.path.exists(HomeDir)==True:   
		...
		dialog = xbmcgui.Dialog()
		dialog.ok(MaintTitle, "Crash logs deleted", "[COLOR yellow]Thank you for using PSMC Maintenance[/COLOR]")
            
	else:
	
		dialog = xbmcgui.Dialog()
		dialog.ok(MaintTitle, "An error occured", "[COLOR yellow]Please report this to PSMC Maintenance[/COLOR]")
...
def viewLogFile():
	...
	if os.path.isfile(kodilog) or os.path.isfile(spmclog) or os.path.isfile(dbmclog):
		...
		dialog.ok(MaintTitle,'Sorry, No log file was found.','','[COLOR yellow]Thank you for using PSMC Maintenance[/COLOR]')
...
def TextBoxes(announce):
	class TextBox():
		...
		def setControls(self):
			self.win.getControl(self.CONTROL_LABEL).setLabel('[COLOR red]PSMC[COLOR snow] -[COLOR blue] Log Viewer[/COLOR]') # set heading
...
def AutoPackages():
    ...
    for root, dirs, files in os.walk(purgePath):
            ...
            if file_count > 0:            
                ...
                for d in dirs:
                    shutil.rmtree(os.path.join(root, d))
    dialog.ok("[B][COLOR yellow]PSMC Maintenance[/COLOR][/B]", "               [COLOR=lime]All Packages have been deleted.[/COLOR]")
