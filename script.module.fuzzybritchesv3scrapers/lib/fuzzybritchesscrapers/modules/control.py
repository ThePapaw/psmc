...
addonObject = addon('script.module.fuzzybritchesv3scrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from fuzzybritchesscrapers.modules import libtools
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.fuzzybritches_v3.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.fuzzybritches_v3.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.fuzzybritches_v3.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.fuzzybritches_v3.artwork)')
...
def textViewer(file=None, text='', heading=addonInfo('name'), monofont=True):
    ...
    if file:
        ...
        txt = text
    if not txt: txt = ' '
    head = '[COLOR orange][I]%s[/I][/COLOR]' % six.ensure_str(heading, errors='replace')
    if getKodiVersion() >= 18: return dialog.textviewer(head, txt, monofont)
    else: return dialog.textviewer(head, txt)


# def metaFile():
    # if condVisibility('System.HasAddon(script.fuzzybritches_v3.metadata)'):
        # return os.path.join(xbmcaddon.Addon('script.fuzzybritches_v3.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
